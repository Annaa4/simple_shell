#include "shell.h"
/**
 * exam_line - Parses the command line looking for commands and argumements.
 ...
 * @cmd_count: A counter keeping track of how many commands have been
 * entered into the shell.
 * @av: Name of the program running the shell
 */
void exam_line(char *line, size_t size, int cmd_count, char **av)
{
  int i;
  ssize_t read_len;
  int token_count;
  ...
  if (read_len != -1)
      ...
      if (param_array[0] == NULL)
	{
      ...
      if (i == -1)
	create_child(param_array, line, cmd_count, av);
...
void create_child(char **param_array, char *line, int count, char **av)
{
  ...
  if (id == 0)
    {
      tmp_cmd = param_array[0];
      cmd = path_finder(param_array[0]);
      if (cmd == NULL)
	{
	  /*Looking for file in current directory*/
	  check = stat(tmp_cmd, &buf);
	  if (check == -1)
	    {
